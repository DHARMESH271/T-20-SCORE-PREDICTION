...
st.title('CRIKET SCORE PREDICTOR')
...
if st.button('Predict Score'):
    ...
    input_df = pd.DataFrame({'batting_team': [batting_team], 'bowling_team': [bowling_team], 'city': [city], 'current_score': [current_score],
     'balls_left': [balls_left], 'wickets_left': [wickets_left], 'crr': [crr], 'last_five': [last_five]})
    result = pipe.predict(input_df)
    st.subheader("PREDICTED SCORE : " + str(int(result[0])))
